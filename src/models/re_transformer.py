...
class ReTransformerTranslation(nn.Module):
    def __init__(self, src_vocab, trg_vocab, d_model, pad_idx=0, N_enc=6, N_dec=2, heads=8, dropout=0.1):
        super().__init__()
        self.pad_idx = pad_idx
        self.encoder = ReEncoder(src_vocab, d_model, N_enc, heads, dropout)
        self.decoder = Decoder(trg_vocab, d_model, N_dec, heads, dropout)
...
def test_model():
    # 1. Khởi tạo các tham số giả lập
    src_vocab_size = 5000
    trg_vocab_size = 5000
    d_model = 512
    pad_idx = 0
    ...
    # 3. Tạo dữ liệu ảo (Tensor số nguyên giả lập ID của từ)
    # Giả lập một số vị trí là pad_idx (0)
    src_data = torch.randint(1, src_vocab_size, (batch_size, src_seq_len)).to(device)
    src_data[:, -5:] = pad_idx # Giả lập 5 token cuối là padding
    
    trg_data = torch.randint(1, trg_vocab_size, (batch_size, trg_seq_len)).to(device)
    trg_data[:, -3:] = pad_idx # Giả lập 3 token cuối là padding

    print(f"Input Shape: {src_data.shape}")
    print(f"Target Input Shape: {trg_data.shape}")
    ...
    # 5. Kiểm tra kết quả
    print("-" * 30)
    print(f"Output Shape: {output.shape}") # Kỳ vọng: (batch_size, trg_seq_len, trg_vocab_size)
    
    expected_shape = (batch_size, trg_seq_len, trg_vocab_size)
    if output.shape == expected_shape:
        print("✅ Test PASSED: Chiều dữ liệu đầu ra chính xác!")
    else:
        print("❌ Test FAILED: Sai chiều dữ liệu đầu ra.")

if __name__ == "__main__":
    # Lưu ý: Bạn cần copy các class thành phần (EncoderLayer, etc.) vào cùng file để chạy được
    test_model()